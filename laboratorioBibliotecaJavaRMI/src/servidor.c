...
void* hilo_auxiliar01(void* arg) {
    ...
    while (ejecucion_receptor) {
        printf("Esperando solicitud del buffer...\n");
        ...
        printf("Procesando solicitud: %c, Libro: %s, ISBN: %d\n",
        ...
        if (solicitud.operacion == 'D') {
            printf("Iniciando proceso de devolución...\n");
            ...
            if (info.linea_ejemplar_estado != -1 && cambiar_estado_libro(archivo_entrada, info.linea_ejemplar_estado, 'D') == 1) {
                ...
                printf("Devolución completada exitosamente\n");
            } else {
                printf("Error en devolución - libro no encontrado o no prestado\n");
                char respuesta[256];
                snprintf(respuesta, sizeof(respuesta), "Error al devolver el libro %s con ISBN %d.\n", solicitud.nombre_libro, solicitud.isbn);
                enviar_respuesta(fd_respuesta, respuesta);
            }
        } else if (solicitud.operacion == 'R') {
            printf("Iniciando proceso de renovación...\n");
            InfoLibro info = buscar_info_libro(archivo_entrada, solicitud.nombre_libro, solicitud.isbn, 'P');
            if (info.linea_ejemplar_estado != -1 && actualizar_fecha_linea(archivo_entrada, info.linea_ejemplar_estado, 1) == 1) {
                ...
                enviar_respuesta(fd_respuesta, respuesta);
                printf("Renovación completada exitosamente\n");
            } else {
                printf("Error en renovación - libro no encontrado o no prestado\n");
                char respuesta[256];
...
void* hilo_auxiliar02(void* arg) {
    ...
    char comando;
    printf("Hilo de comandos iniciado\n");
    ...
    while (ejecucion_receptor) {
        ...
        if (comando == 'r') {
            printf("Generando reporte de operaciones...\n");
    ...
    printf("Hilo de comandos finalizado\n");
    return NULL;
}
...
int main(int argc, char *argv[]) {
    ...
    printf("Iniciando bucle principal de recepción\n");
    while(ejecucion_receptor) {
        printf("Esperando solicitud del pipe...\n");
        ...
        printf("Solicitud recibida: %c, Libro: %s, ISBN: %d\n",
               solicitud.operacion, solicitud.nombre_libro, solicitud.isbn);
        ...
        if (solicitud.operacion == 'Q') {
            ...
            printf("Procesando solicitud de préstamo\n");
            ...
            printf("Enviando solicitud %c al buffer para procesamiento por hilo auxiliar\n", solicitud.operacion);
            sem_wait(&espacios_disponibles);
            sem_wait(&acceso_buffer);

            Buffer[in] = solicitud;
            in = (in + 1) % BUFFER_SIZE;
            printf("Solicitud agregada al buffer en posición %d\n", (in - 1 + BUFFER_SIZE) % BUFFER_SIZE);
